...
The data for this case study contains 100 services over 365 days and commences on `r format(start, "%d %B %Y")`, to avoid any confusion with reality. The `as.Posixct()` function formats the information as a date-time field with the appropriate time zone. In most smart meter systems, data is time-stamped to UTC (Coordinated Universal Time) to avoid confusion with changes in time.
...
ggsave("manuscript/resources/12_time_series/model-diurnal.png", width = 6, height = 4)
...
In this simulation, only 10% of water consumers is home all day. The `home` vector stores for which services consumers are home (1) or away (0). The `hols` vector indicates those 10% of customers who go on a holiday during the simulated period. A zero indicates that that the people at this service are on holidays. 

A [binomial distribution](https://en.wikipedia.org/wiki/Binomial_distribution) simulates these variables. This type of distribution is ideal for events that are like coin flips. The `rbinom()` function generates `n` random variables. The second number indicates the number of 'flips' and the last parameter indicates the probability of success. For example, using `c("heads", "tails")[rbinom(100, 1, .5) + 1]` simulates 100 coin flips with an unbiased coin.
