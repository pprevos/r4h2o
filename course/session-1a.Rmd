...
<div class="cols">
![](images/autodesk-water-unicorns.jpg){height=400}
...
Script -> REPL [constraint=false]
REPL -> Console 
REPL -> File
REPL -> Graphic
REPL -> HTML}", width=900, height=600)
```
...
- Volume: $V = \frac{\pi}{4} D^2 \big( d_1 + \frac{d_2}{3} \big)$
- Flow: $Q=4 m^3/h$
...
## Variable Classes
- Integer (${\ldots -3, -2, -1, 0, 1, 2, 3, \ldots}, \mathbb{N}$)
- Numeric ($\mathbb{R}$)
- Complex numbers ($a + bi, \mathbb{C}$)
- Dates (`"`r Sys.Date()`"`)
- Text (`"abcd"`)
- Boolean (`TRUE`, `FALSE`)
- Factors (classifications): (`"Male", "Female", "Other"`)

Conversions: `as.numeric()`, `as.character`, `as.Date()`.
...
Open `scripts/04-statistics.R`K
...
There are nine possible ways to interpolate the data. Run `?quantile` and read the help file.
