...
Now that we have covered the basics of the R language, it is time to analyse some data. The next four sessions use data from the first case study. This case study is about assessing water quality data from a reticulation network for compliance with regulations. We analyse this data using extended functionality in R. This course mainly uses the libraries of the Tidyverse collection.

This chapter has the following learning objectives:
* Understand the principles of R packages and libraries.
* Install and activate the Tidyverse libraries.
* Load a data set (CSV) and describe the data.
...
Thousands of specialised packages undertake a vast range of complex tasks. You can, for example, use R as a GIS and analyse spatial data, or implement machine learning. Other packages help you to access data from various sources, such as SQL databases. R packages can also help you present the results of your analysis as a presentation, report or interactive dashboard.

The majority of R packages are available on [CRAN](https://cran.r-project.org/), the *Comprehensive R Archive Network*. You can install packages in R with the `install.packages()` function. Within RStudio you can install packages in the *Tools* menu. When you install a package, RStudio downloads a series of files and stores them for future use. This collection of files is a library. The words package and library are sometimes used interchangeably. In R, a package is a collection of R functions, data and compiled code. The location where the packages are stored is called the library.
...
One of the most popular collections of R packages for analysing data is the [Tidyverse](https://www.tidyverse.org/), developed by R guru Hadley Wickham and many others. Doing 'tidy' data science has a strong following. Tidy data science relates to writing code and cleaning data in a way that promotes reproducibility.

The Tidyverse packages provide additional functionality to extract, transform, visualise and analyse data. The features offered by these packages are more versatile, easier to use and understand than the base R code.
...
Some data scientists prefer not to load the complete set of packages and choose to load each one separately to spare computer resources. 
...
## Case Study 1
The first case study looks at water quality data in the eleven cities in Gormsey. Each town has a set of sample points at the customer tap in their garden. The alchemist regularly samples these taps and tests the water for a range of parameters in her laboratory. She stores all results in a CSV file, which we use for this case study. In reality, water utilities sample hundreds of different parameters at different frequencies at the water treatment plant and the water network. Analysing this data is a common activity for water professionals. The data for this case study only contains three parameters.
...
You can also watch the screencast that demonstrates the functionality explained in this chapter. The best way to learn is to type the expressions in this chapter as you read or view the video. A productive way to comprehend the information is to change the examples and study the output.
...
You can find see the current working directory with the `getwd()` function. Without a working directory, you would have to specify the complete path, such as: `C:/Users/peterp/Documents/r4h2o/casestudy2/gormsey.csv`. In this example, the working directory is `C:/Users/peterp/Documents/r4h2o/`. 

The `read_csv()` function assumes that the first row contains the field names and the following rows contain the data, organised in columns.
...
R can read many types of data. Some specialised extensions can connect R to Excel spreadsheets, SQL databases, scrape data from websites, GIS layers, and many other sources. The data for this case study uses real laboratory data that was anonymised and modified to make it more interesting. The `extract_data.R` file in the case study folder shows how the turbidity data was extracted from a corporate SQL server, using the RODBC package.
...
I> Answer: To find the last element of the data frame, use the `nrow()` function within square brackets.
...
Variables that are either TRUE or FALSE are called logical, and they are the building blocks of computer science, the famous ones and zeroes. These variables are useful to indicate conditions. These variables can also be used in calculations. The code below results in a vector with the values 2 and 0.
...
![Figure 4.1: Filtering a data frame: `filter(df, var == "B")`](resources/04_tidyverse/filter.png)
...
These relations result in a Boolean value of TRUE or FALSE, for example `1 > 2` results in `FALSE`. R also evaluates relations between character strings, using alphabetical order. In R, `"small" > "large"` results in TRUE because *s* comes after *l*.

When you apply these operators to a vector, then R assesses all elements in the vector. The expression `c(2, 3) > 2` results in a vector with TRUE and FALSE as elements.
...
The dplyr package of the Tidyverse library has  `filter()` function that makes filtering data frames a bit easier. The first parameter is the data frame, and the second parameter is the filtering condition. There is no need to use the `$` indicator, e.g. `filter(gormsey, Measure == "Turbidity")`.
...
I> Answer: We subset the data for all results less than 0.1 NTU and where the Town is not Bealiba. The nrow function counts the results.
...
You now have seen a series of fictions that you can use to load and explore data. With this knowledge, you can now complete the next five quiz questions.
