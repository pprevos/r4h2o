...
youtube("uHr71iUySaA", "Initial setup for RStudio desktop version.")
```
...
```{r, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
youtube("NyOY620JHyw", "Initial setup for RStudio cloud version.")
```
...
When you open RStudio for the first time, the window consists of three panes, each with various tabs. The left panel is the console. The top-right pane shows the system environment and the one below that shows a list of files and folders (Figure \@ref(fig:rstudio)).
...
```{r rstudio, fig.cap="RStudio cloud version default screen layout", echo=FALSE, out.width="100%"}
knitr::include_graphics("images/rstudio.png")
```

```{r, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
youtube("ZK57uRg9l5Y", "Basic principles of the R language.")
```
...
The modulo function returns the reminder of a division, e.g. `17 %% 5` results in 2. Integer division returns the truncated result of a division, e.g. `17 %/% 5` results in 3.

Furthermore, R is 'meme-proof' because it knows the correct answer to the many arithmetic memes distributed on social media (Figure \@ref(fig:meme)).

```{r meme, fig.cap="Arithmetic facebook meme.", echo=FALSE, out.width="40%"}
knitr::include_graphics("images/memes.jpg")
```
...
* `abs(x)`: Absolute value ($\mid x \mid$)
* `exp(x)`: Exponential ($\mathrm{e}^x$)
* `factorial(x)`: Factorial ($x!$)
* `log(x, base = y)`: Logarithm ($\log_{y} x$, $y = \mathrm{e}$ by default)
* `sqrt(x)`: Square root ($\sqrt x$)

All these functions take a single number or a vector as input. The `length()` function determines the number of elements in a vector.
...
Some calculations resulted in either `NaN` or `Inf`. The first indicator means 'Not a Number', which occurs when you try the factorial or logarithm or square root of a negative number. R can calculate with complex numbers, but you need to indicate this as such, for example: `sqrt(as.complex(-23))`. 

### Basic plots

R has extensive capability for visualising data and the results of analysis. The last section shows some examples of simple graphs (figure \@ref(fig:basic-plot)).

The first few lines defines the variables `x` and `y` and plots them as a line (`type = "l"`), showing the parabola in the plot window. The `abline()` function draws a straight line on top of the current plot. 

The `barplot()` function creates a bar chart with the complaints vector. Note how the function parameters are split over two lines to make the code easier to read.

```{r basic-plot1, fig.cap="Basic R Plots.", fig.show="hold", out.width="50%"}
x <- -10:10
y <- -x^2 -2 * x + 30
...
```{r basic-plot2, fig.cap="Basic R Plots.", fig.show="hold", out.width="50%"}
complaints <- c(12, 3, 23, 45, 2, 99, 1)
barplot(complaints, 
...
When you hit enter within a script, the editor adds a new line. To execute a line of code in the editor, you need to type Control-Enter or t he Run button on top of the window. When you hit the Source button, RStudio evaluates all code in the script. You can also select a section of code or part of a line and only run that part.
...
> Open the help file for the `plot()` function. How do you plot a function with both points and lines?
...
To calculate the flow, you can use a simplified version of the Kindsvater-Carter rectangular weir equation ([ISO 1438: 2017](https://www.iso.org/standard/66463.html)):
...
The value for $C_d$ is an estimate because it depends on the dimensions of the weir and the flow characteristics. Follow [this link](https://www.engineeringexcelspreadsheets.com/tag/kindsvater-carter-formula/) for a detailed discussion on using this formula. The photo below shows what such a weir looks like in practice.

```{r weir, fig.cap="Example of a channel with a rectangular weir.", echo=FALSE, out.width="60%"}
knitr::include_graphics("images/weirplate.jpg")
```
...
Tip: create a vector of height measurements with `c()` to use the formula only once. Don't forget to convert the units (1 m^3^/s = 1000 L/s). You can use the `mean()` function to average the results in a vector.
...
The [Base R Cheat Sheet](https://raw.githubusercontent.com/rstudio/cheatsheets/master/base-r.pdf) provides a comprehensive list of basic functionality in the R language. Some of these functions have not been covered in this chapter but I encourage you to have a play with everything on this sheet.
